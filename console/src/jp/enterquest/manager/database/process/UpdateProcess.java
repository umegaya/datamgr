 ...
 */
public final class UpdateProcess extends Process
{
	/**
	 * コンストラクタ
	 * @param server HTTPサーバ
	 */
	public UpdateProcess(final HttpServer server)
	{
		super(server);
	}
	...
	{
		final Process next_process = this.update(request);
	 ...
	 * @return 次の処理
	 */
	private final Process update(final HttpServerRequest request)
	{
		...
		try
		{
			...
			{
				logger.info("client=%s operator=%s database=%s table=%s : update row.", request_client, request_operator_name, request_database_name, request_table_name);
			}
			...
			try
			{
				...
				for (final InformationColumn.Row column : columns)
				{
					...
					if (request.hasParameter(request_key))
					{
						...
						{
							values.set(column_name, Common.getInstance().getRandomValue(update_connection, table, columns));
						}
				...
				for (final InformationKey.Row key : keys)
				{
					if (key.isPrimary())
					{
						...
						String request_key = String.format("key:%s", column_name);
						if (request.hasParameter(request_key))
						{
							final Data request_value = request.getParameter(request_key);
